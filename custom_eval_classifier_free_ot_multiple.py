...
Ws = [0, 6.5] # 0.1, 0.5, 1, 2, 3, 5, 6.5, 10]
method = 'average_vector'
method = 'average_embedding'
n_negativess = [2,8] # 4,6,
...
for n_negatives in n_negativess:
    ...
    def plot_classifier_free_from_matching(explore_name : str, 
        ...
        for i in range(len(src.data) // n_images_per_row):
            ...
            plt.axis('off')
            os.makedirs(os.path.join(f'classifier_free_matching_multiple', explore_name), exist_ok=True)
            plt.savefig(os.path.join(f'classifier_free_matching_multiple', explore_name, f'sample_{i}.png'))
            plt.close()
