...
class LinearTimeCrossAttention(eqx.Module):
    ...
    def __call__(self, x, cond, key=None):
        ...
        # Notice that in our case that (h w), i.e height x witdh do not coincide for q and k,v
...
# Notice the Resnet Blocks are simple modifications from ResnetBlocks to allow either FiLM or CrossAttention. 
# We could move all into a single parametrized ResnetBlock, for now to allow for more versatility to modify (as the expense of code duplication) leave it separate.
...
class CrossAttentionResnetBlock(eqx.Module):
    dim_out: int
    is_biggan: bool
    up: bool
    down: bool
    ...
    def __init__(
        ...
        heads,
        dim_head,
        cond_dim, # ! Only change from our standard residual block 
        *,
        key,
    ):
        keys = jax.random.split(key, 7)
        ...
        if is_attn:
            self.attn = CondResidual( # ! Only change from normal residual block!
...
if __name__ == "__main__":  # Debug area
    ...
    if test_film:
        ...
        # Verify FiLM transformation (for various points in same channel)
