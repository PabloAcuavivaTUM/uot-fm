...
def get_model(
    ...
    if config.model.type == "mlpmixer":
        ...
        tgts_list = config.data.get("type_tgt", [])
        perturbation_embedding_dim = len(tgts_list)
        return UNet(
            in_data_shape,
            out_data_shape,
            ...
            film_cond_dim=config.model.film_cond_dim + config.training.get("genot", dict()).get("use_fb_embedding", 0) + perturbation_embedding_dim, 
            ###
            key=model_key,
        )
    else:
