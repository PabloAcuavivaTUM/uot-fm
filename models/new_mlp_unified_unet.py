...
class UNet(eqx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if np.any(film_middle) or np.any(film_resolutions_up) or np.any(film_resolutions_down):
            # Experimental, MAKE FiLM more powerful as a "style conditioning"
            self.mlp_film = eqx.nn.MLP(
                in_size=film_cond_dim,
                out_size=film_cond_dim,
                width_size=2*film_cond_dim,
                depth=1, 
                activation=jax.nn.silu,
                key=keys[1],
            )
        ...
        self.first_conv = eqx.nn.Conv2d(
            in_data_channels, hidden_size, kernel_size=3, padding=1, key=keys[2]
        )
        ...
        self.final_conv_layers = [
            ...
            jax.nn.silu,
            eqx.nn.Conv2d(hidden_size, out_data_channels, 1, key=keys[7]),
        ]
    ...
    def __call__(self, t, x_t, film_cond, cross_attn_cond, *, key=None):
        ...
        if self.mlp_film is not None:
            film_cond = self.mlp_film(film_cond)
